        ...
        <div class="fixed inset-y-0 right-0 max-w-sm w-full flex">
          <div class="w-full h-full bg-white dark:bg-slate-900 shadow-2xl flex flex-col animate-[slideInRight_0.3s_ease-out]">
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-6">
              ...
              <div v-if="currentUser?.role === 'admin' || currentUser?.role === 'moderator' || currentUser?.role === 'digital_marketer'" class="bg-indigo-50 dark:bg-indigo-900/10 p-4 rounded-xl border border-indigo-100 dark:border-indigo-800/50">
                ...
                <div class="space-y-3">
                  ...
                  <div class="flex gap-2">
                    <select v-model="newTask.assigneeRole" class="flex-1 px-3 py-2 text-sm rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:border-indigo-500 outline-none transition-all">
                      ...
                      <option value="digital_marketer">Digital Marketers</option>
                    </select>
...
// Insert the panel right before the closing </div> of the app root
html = html.replace('<!-- End App Container -->', tasksPanelHtml + '\n      <!-- End App Container -->');
