...
const target1 = `      logHistory(payloadObj, stats);
      return ContentService.createTextOutput(JSON.stringify({ status: 'success', mode: 'delta'`;

const replace1 = `      logHistory(payloadObj, stats);
      try { distributeOrdersBySeller(); } catch(e) {}
      return ContentService.createTextOutput(JSON.stringify({ status: 'success', mode: 'delta'`;

code = code.replace(target1, replace1);

const target2 = `    logHistory(payloadObj, stats);
    return ContentService.createTextOutput(JSON.stringify({ status: 'success', mode: 'full'`;

const replace2 = `    logHistory(payloadObj, stats);
    try { distributeOrdersBySeller(); } catch(e) {}
    return ContentService.createTextOutput(JSON.stringify({ status: 'success', mode: 'full'`;
